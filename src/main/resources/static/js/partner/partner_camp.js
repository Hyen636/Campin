...
camp.modify = function(){
	let param = $("#frm_camp_modify").serialize();
	$.ajax({	
		data : param,
		url : "partner_input_camp_hitory",
		type : "post",
		...
		success : function(resp){
			$("input[name=hisItemSno]").val(resp);
			param = $("#frm_camp_modify").serialize();
			$.post("partner_input_filter_hitory", param, function(){									
				let tempFrm = $("#frm_att")[0];
				let frmAtt = new FormData(tempFrm);
				$.ajax({
...
camp.confirm = function(){
	$("input[name=state]").val("등록완료");
	let frm = document.getElementById("frm_camp_view")
	let param = $(frm).serialize();
	$.ajax({	
		data : param,
		url : "manager_camp_confirm",
		type : "post",
		cache : false,
...
camp.reject = function(){
	$("input[name=state]").val("등록반려");
	window.open("manager_camp_pop", 'window', 
...
camp.geo = function(result, status) {
	if (status === kakao.maps.services.Status.OK) {
		...
		$("input[name=latitude]").val(result[0].x);
	}
};
// 배치도
